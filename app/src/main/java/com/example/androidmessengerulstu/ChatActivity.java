...
import com.example.androidmessengerulstu.databinding.ActivityChatBinding;
import com.example.androidmessengerulstu.message.Message;
import com.example.androidmessengerulstu.message.MessagesAdapter;
        ...
        FirebaseDatabase.getInstance().getReference().child("Chats").child(chatId)
                ...
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful()) {
                        // Если сообщение успешно отправлено, прокручиваем RecyclerView в конец
                        binding.messagesRv.scrollToPosition(binding.messagesRv.getAdapter().getItemCount() - 1);
                    }
                });
    }
